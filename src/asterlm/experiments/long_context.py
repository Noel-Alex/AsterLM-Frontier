...
def build_exact_key_case(
    ...
    *,
    ...
    fixed_tokens = len(payload_ids) + len(query_ids) + len(answer_ids)
    ...
    before_count = min(filler_count, max(0, round(filler_count * depth)))
    ...
    prompt_ids = (*filler[:before_count], *payload_ids, *filler[before_count:], *query_ids)
    answer_tuple = tuple(answer_ids)
    if len(prompt_ids) + len(answer_tuple) != target_sequence_tokens:
        raise AssertionError("long-context case construction lost exact token accounting")

    context_tokens = filler_count + len(payload_ids)
    actual_depth = before_count / max(context_tokens - len(payload_ids), 1)
    stable = (
...
def summarize_retrieval_results(results: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    if not completed:
        return {"completed_cases": 0, "exact_greedy_accuracy": None, "by_length": {}}
