...
def test_remote_gpu_profiles_preserve_effective_batch_geometry() -> None:
    ...
    train = ROOT / "configs/train/frontier_100b_stage1_8k.yaml"
    expected = {"L40S": (2, 8), "A100-80GB": (4, 4), "H200": (8, 2), "B300": (16, 1)}
    for gpu, geometry in expected.items():
        ...
        assert cfg["optimizer"] == "muon_adamw"
...
def test_gcp_machine_types_resolve_to_gpu_execution_profiles() -> None:
    ...
    train = ROOT / "configs/train/frontier_100b_stage1_8k.yaml"
    expected = {
        "g2-standard-4": ("L4", 1, 16),
        "a2-ultragpu-1g": ("A100-80GB", 4, 4),
        "a3-highgpu-1g": ("H100-80GB", 4, 4),
    }
