...
class LatentMoE(nn.Module):
    ...
    aggregate is RMS-normalized before the up-projection as a stability measure used by
    modern latent-MoE designs. Standard SwiGLU is retained so the first experiment
    ...
    def __init__(
        ...
        # Keep the routed path faithful to LatentMoE Eq. (1)/(2): project down,
        # expert compute/aggregate in latent space, then project back up. No extra
        # post-expert normalization is inserted here, so architecture comparisons
        # do not silently add an unreported stabilizer.
        del norm_eps
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        routed_out = self.up_proj(routed_latent)
